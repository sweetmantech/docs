...
```python [Python - Existing Collection]
import requests
...
```javascript [JavaScript - Existing Collection]
...
```typescript [TypeScript - Existing Collection]
...
```javascript [JavaScript - New Collection]
const response = await fetch("https://inprocess.world/api/moment/create", {
  method: "POST",
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({
...
```typescript [TypeScript - New Collection]
interface SalesConfig {
  type: "fixedPrice" | "erc20Mint";
  pricePerToken: string;
  saleStart: string; // Unix timestamp as string
...
To learn more about Splits Configuration, see the [Splits Configuration Reference](/moment/splits).
...
- **salesConfig**: Configuration for how the token can be sold (see [Sales Configuration](/moment/sales-config) below)
...
To learn more about SalesConfig, see the [SalesConfig Reference](/moment/create/sales-config).
...
This endpoint accepts a simplified schema optimized for text content. See the [Writing Moments Documentation](/moment/writing) for details.
