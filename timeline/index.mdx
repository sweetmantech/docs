...
```javascript [JavaScript - Collection]
async function getTimeline(params = {}) {
  const {
    artist,
    collection,
...
```typescript [TypeScript - Collection]
export interface Admin {
  address: string; // Address
  username: string; // Username of the admin/artist, if available
  hidden: boolean; // Whether the moment is hidden by this admin
...
```javascript [JavaScript - in•process]
async function getTimeline(params = {}) {
  const {
    artist,
    collection,
...
| moments[].max_supply             | number \| null | Maximum supply of tokens (null if unlimited)                                                                      |
