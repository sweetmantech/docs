...
export const loginWithOTP = async (
  email: string,
  code: string
): Promise<{ token: string; social_wallet: string }> => {
  ...
  return { token: data.token, social_wallet: data.social_wallet };
};
```
...
export const useInProcessAuth = () => {
  ...
  const [socialWallet, setSocialWallet] = useState<string | null>(null);
  ...
  const handleLogin = async (code: string) => {
    ...
    setLoading(true);
    try {
      const { token: jwt, social_wallet } = await loginWithOTP(email, code);
      setToken(jwt);
      setSocialWallet(social_wallet);
  ...
  return { step, email, setEmail, token, socialWallet, error, loading, handleSendCode, handleLogin, reset };
};
```
...
{
  "token": "<JWT_TOKEN>",
  "social_wallet": "<PRIVY_EMBEDDED_WALLET_ADDRESS>"
}
```
...
- Use the token to [create moments](/api-reference/moment/create) or manage [collections](/api-reference/collections/create) on behalf of the signed-in artist
